...
public class DirectByteBufferIndexInput extends IndexInput implements RandomAccessInput {
    ...
    public static DirectByteBufferIndexInput newInstance(
        ...
        DirectByteBufferIndexInput input = new DirectByteBufferIndexInput(
                resourceDescription, 0L, length, vfd, blockCache, false, blockTable);
     ...
     *
     * <p>The fast path checks {@code pos < currentBlockEnd} then reads via
     * {@code MemorySegment.get()}. Combined with eager seek (which resolves
     * the block at seek time), this ensures the fast path is always hit
     * during sequential reads, giving C2 enough profiling iterations to
     * observe the outer loop exit and avoid unstable_if deoptimization.
     */
    @Override
    ...
    DirectByteBufferIndexInput buildSlice(String sliceDescription, long sliceOffset, long length) {
        ...
        DirectByteBufferIndexInput slice = new DirectByteBufferIndexInput(
                desc, sliceAbsoluteBase, length, vfd, blockCache, true, blockTable);
    ...
    public final void close() throws IOException {
        ...
        if (!isClone) {
            blockTable.clear();
            blockCache.deregisterTable(vfd);
            VirtualFileDescriptorRegistry.getInstance().deregister(vfd);
        }
    }
}
