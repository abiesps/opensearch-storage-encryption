...
public class DirectByteBufferBlockLoader implements BlockLoader<RefCountedMemorySegment> {
    ...
    public DirectByteBufferBlockLoader(KeyResolver keyResolver, EncryptionMetadataCache encryptionMetadataCache) {
        this(keyResolver, encryptionMetadataCache, new DirectMemoryAllocator());
    }
    ...
    public RefCountedMemorySegment[] load(Path filePath, long startOffset, long blockCount, long poolTimeoutMs) throws Exception {
        ...
        try (
            ...
            while (blockIndex < blockCount && bytesCopied < bytesRead) {
                ...
                // Required operation (on-demand block read): wrap MemoryBackPressureException
                // in OutOfMemoryError so the Lucene layer sees the system cannot serve the read (Req 13.4)
                ByteBuffer directBuf;
                try {
                    directBuf = allocator.allocate(toCopy);
                } catch (MemoryBackPressureException mbpe) {
                    // Clean up any already-allocated handles before throwing (Req 13.5)
                    ...
                    OutOfMemoryError oome = new OutOfMemoryError("Cannot allocate block: " + mbpe.getMessage());
                    oome.initCause(mbpe);
                    throw oome;
                }
